...
def MSM_starting_values_pf(data,startingvals,kbar,A_template,B):
    if startingvals is None:
        print("No starting values entered: Using grid-search")
        ...
        for i in range(lb):
            for j in range(lg):
                xopt,fval,ierr,numfunc = scipy.optimize.fminbound(particle_filter,
                                                 x1 = m0_lower,x2 = m0_upper,xtol = 1e-3,
                                                 args = (kbar,data,A_template,B,[b[i],g[j],sigma]),full_output = True)
                m0,LL = xopt,fval
                output_parameters[idx,:] = b[i],m0,g[j]
                LLs[idx] = LL
                idx +=1
